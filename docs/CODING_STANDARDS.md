...
# Terra C Coding Standards

This document defines the mandatory coding standards for the Terra text editor project. All code contributions must follow these rules for consistency, readability, and maintainability.

---

## Purpose

These standards ensure that Terra's codebase is:
...
- **Examples:** `TERRA_TAB_STOP`, `MAX_BUFFER_SIZE`, `CURSOR_MOVE`
- **Rule:** All preprocessor macros and constants use uppercase with underscores

### Functions, Variables, Structs, and Typedefs
- **Format:** `snake_case`
...
- Type-hint suffixes like `_t` for custom typedefs (exception: standard C/POSIX types like `size_t`, `ssize_t`)
...
- Cryptic type names (`erow`, `abuf` - use `editor_row`, `append_buffer`)
...
- Struct/typedef names with abbreviated words (`erow` → `editor_row`)
...
- **Never use tabs**
- Mixed spaces/tabs will be rejected
...
```c
// Correct
int row_char_index_to_render_index(editor_row *row, int char_index);
void buffer_insert_char(buffer_state *buffer, int position, char character);

// Wrong - never wrap signatures
int row_char_index_to_render_index(editor_row *row,
                                   int char_index);
void buffer_insert_char(buffer_state *buffer,
                        int position,
                        char character);
```
...
Write comments like you're explaining code to a colleague. Vary your sentence structure and avoid robotic repetition. You can use first-person ("I", "we"), direct descriptions, or passive voice - whatever sounds most natural for that particular comment.
...
```c
// Correct - each field explained naturally
...
void row_destroy(editor_row *row);
...
**After (correct - single line, descriptive names):**
...
int row_render_index_to_char_index(editor_row *row, int render_index);
```

### Example 5: Struct Formatting and Comments

**Before (wrong - trailing comments, cryptic names, poor formatting):**
...
**After (correct - comments above, descriptive names, consistent formatting):**
...
### Type Names (struct, typedef, enum)
- [ ] Scan for cryptic struct/typedef names
  - BAD: `erow`, `abuf`, `cfg_t`
  - GOOD: `editor_row`, `append_buffer`, `configuration`
...
Use this checklist when reviewing code or refactoring existing code to ensure compliance with Terra standards.
...
8. **Remove prohibited patterns** — no new `_t` typedefs, no `g_` globals, no cryptic abbreviations
...
- [ ] No custom `_t` suffix for typedefs
...
- [ ] Comments use first-person perspective ("I..." or "We...")
...
These standards exist to make Terra's codebase:
